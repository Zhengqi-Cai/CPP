...
customLogging::Logger customLogging::StaticLogger::_logger;


//----< send text message to std::ostream >--------------------------

void customLogging::Logger::write(bool logOpen, const std::string& msg)
{
	if (logOpen) {
		if (_ThreadRunning)
			_queue.enQ(msg);
...
void customLogging::Logger::title(bool logOpen, const std::string& msg, char underline)
{
	if (logOpen) {
...
//Logger & Logger::operator<<(const boost::any pObj)
//{
//	for each (auto stream in streams_)
//	{
//		*stream << *pObj;
//	}
//	return *this;
...
void customLogging::Logger::attach(std::ostream* pOut)
{ 
 ...
 */
void customLogging::Logger::start()
{
  if (_ThreadRunning)
    return;
  _ThreadRunning = true;
...
bool customLogging::Logger::running()
{
  return _ThreadRunning;
}
//----< suspend logger >---------------------------------------------

void customLogging::Logger::pause(bool doPause)
{
  if (doPause)
  {
    _Paused = true;
...
bool customLogging::Logger::paused()
{
  return _Paused;
}
//----< wait for logging to empty input queue >----------------------

void customLogging::Logger::flush()
{
...
void customLogging::Logger::stop(const std::string& msg)
{
  if (_ThreadRunning)
  {
    if(msg != "")
      write(true, msg);
    write(true, "quit");    // request thread to stop
    if (_pThr->joinable())
...
void customLogging::Logger::wait()
{
...
using Util = oldUtilities::StringHelper;
