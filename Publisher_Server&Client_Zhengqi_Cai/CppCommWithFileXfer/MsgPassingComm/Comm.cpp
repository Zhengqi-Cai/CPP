...
#include "../commUtilities/commUtilities.h"
...
using SUtils = commUtilities::StringHelper;
...
bool Sender::sendFile(Message msg)
{
  if (!msg.containsKey("file"))
    return false;
  std::string fileSpec =  msg.file();//file contains fullFileSpec
...
public:
  ...
  */
  ...
  {
    std::string fileName = FileSystem::Path::getName(msg.file());
...
void ThreadProcClnt1()
{
  ...
  for (size_t i = 0; i < IMax; ++i)
  {
    ...
    msg.name("client #1 : msg #" + commUtilities::Converter<size_t>::toString(i));
...
void ThreadProcClnt2()
{
  ...
  {
    Message msg(serverEP, clientEP);
    msg.name("client #2 : msg #" + commUtilities::Converter<size_t>::toString(i));
    std::cout << "\n  " + comm.name() + " posting:  " << msg.name();
    comm.postMessage(msg);
    Message rply = comm.getMessage();
    std::cout << "\n  " + comm.name() + " received: " << rply.name();
...
void DemoClientServer()
{
  ...
  while (true)
  {
    ...
    else  // non-file message
    {
      rply.to(msg.from());
      rply.from(serverEP);
      rply.name("server reply #" + commUtilities::Converter<size_t>::toString(++count) + " to " + msg.from().toString());
