...
{
	return address + ":" + commUtilities::Converter<size_t>::toString(port);
}
...
EndPoint EndPoint::fromString(const std::string& str)
{
	...
	ep.port = commUtilities::Converter<size_t>::toValue(portStr);
	return ep;
}
...
size_t Message::contentLength()
{
  if (containsKey("content-length"))
  {
    ...
    return commUtilities::Converter<size_t>::toValue(lenStr);
  }
  return 0;
}
//----< set body length >----------------------------------------------
...
{
  attributes_["content-length"] = commUtilities::Converter<size_t>::toString(ln);
}
...
Message Message::fromString(const std::string& src)
{
  ...
  std::vector<std::string> splits = commUtilities::StringHelper::split(src);
...
std::ostream& Message::show(std::ostream& out)
{
  ...
  if (StaticLogger::running()) {
	  StaticLogger::write(logOn_, "\n" + temp+"\n");
  }
  else {
	  out << "\n" << temp+"\n"; // prepend newline
  }
