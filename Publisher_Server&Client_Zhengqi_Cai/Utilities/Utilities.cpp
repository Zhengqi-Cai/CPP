...
*/
void StringHelper::Title(bool logOpen, std::string src)
{
	std::ostringstream os;
	os << "\n  " << src;
	os << "\n " << std::string(src.size() + 2, '=');
	StaticLogger::write(logOpen, os.str());
}
...
void StringHelper::sTitle(bool logOpen,std::string src, size_t offset, size_t width, char underline)
{
	std::ostringstream out;
  if (width < src.size())
    width = src.size();
...
void oldUtilities::putline(bool logOpen)
{
	StaticLogger::write(logOpen, "\n");
}


#ifdef TEST_UTILITIES

using Utils = oldUtilities::StringHelper;

int main()
{
  Utils::Title(true,"Testing Utilities Package");
  putline(true);

  Utils::title(true,"test StringHelper::split(std::string)");
  ...
  Utils::title(true,"test std::string Converter<T>::toString(T)");
  ...
  Utils::title(true,"test T Converter<T>::toValue(std::string)");
