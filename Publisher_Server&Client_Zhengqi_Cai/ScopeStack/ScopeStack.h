...
namespace CodeAnalysis
{
  ...
  void ScopeStack<element>::push(const element& item)
  {
	...
	StaticLogger::write(logOpen_,"\n--- stack size = " + oldUtilities::Converter<size_t>::toString(size()) + " ---");
	StaticLogger::flush();
  }
  ...
  template<typename element>
  void showStack(bool logOpen,ScopeStack<element>& stack, bool indent = true)
  {
    if (stack.size() == 0)
    {
  ...
  template<typename element>
  void TreeWalk(bool logOpen,element* pItem, bool details = false)
  {
