...
void GrammarHelper::showParse(const std::string& msg, const Lexer::ITokenCollection& se, bool isResult)
{
  if (StaticLogger::running())
  {
    ...
    //  out << "\n    ";
    out << temp;
    if (isResult && StaticLogger::running())
		StaticLogger::write(logOpen_,out.str());
    else if (StaticLogger::running())
		StaticLogger::write(logOpen_,out.str());
  }
}
...
void testHelpers(const Semi& seIn)
{
  ...
  oldUtilities::StringHelper::title("Testing GrammarHelpers");
  std::cout << "\n  " << show(se);
  se.trim();
...
int main()
{
  oldUtilities::StringHelper::Title("Testing Grammar Functions");
  std::cout << "\n  Note:";
